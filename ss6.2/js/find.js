...
function buy(index) {
    if (confirm("Bạn có chắc chắn với việc nuôi nó không?")) {
        if (localStorage.getItem(products[index].id) !== null) {
            ...
            const purchasedAnimal = products[index];
            newlist.push(purchasedAnimal);
            ...
            usersData[userId].push(purchasedAnimal);
            localStorage.setItem('users', JSON.stringify(usersData));
        }
    } else {
        confirm("Cảm ơn bạn đã xác nhận!");
    }
}
